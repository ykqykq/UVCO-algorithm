...
public class transmissionspeed {
	...
	public static double gettransmissionspeed(int vid,int type){//type :0代表车辆与基站传信息，1代表车辆与无人机传信息
		...
		if(type==0){//车辆与基站通信
		   ...
		   totalchannel=transmissionspeed.allchannel(type);
		   ...
		   //System.out.println("基站：chann:"+df.format(chann)+"   totalchannel:"+df1.format(totalchannel)+"   transspeed:"+df.format(transspeed));
			...
			distance=channelgain.getdistance(main.vehiclestation[vid][0], main.vehiclestation[vid][1], 0, uav.x, uav.y, uav.z);
	...
	public static double allchannel(int type){//计算无人机或者基站的其他车辆干扰0：基站     1：无人机
		double totalchannel=0;
		int v;
		if(type==0){//基站的干扰
			//System.out.println("进来了");
			for(double taid:basestationthread.tasklist.keySet()){
			...
			//System.out.println("进来了");
			for(double taid:uavthread.tasklist.keySet()){
				
				v=uavthread.tasklist.get(taid).origin;
				double distance=channelgain.getdistance(main.vehiclestation[v][0], main.vehiclestation[v][1], 0, uav.x, uav.y, uav.z);           
				//System.out.println("其他车辆的距离"+distance);
				double ch=channelgain.getchannelgain(distance);
				totalchannel=totalchannel+ch;
			}
