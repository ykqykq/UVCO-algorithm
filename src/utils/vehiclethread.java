...
public class vehiclethread implements Runnable{
    static boolean[] havetask=new boolean[500];//用来标志线程是否等待任务再执行false代表当前没有任务true代表当前有任务在执行
	...
	public void run(){
		...
		TreeMap<Double, Location> treeMap = readtraffic.treeMapArray.get(v.id);//存储该车辆的轨迹
		...
		task ta=null;
		time t=new time();
		double taskwait=1000;//车辆产生任务的时间间隔最少一秒
		while(true){
			try {
				Thread.sleep(1);
			...
			if(!vehiclethread.havetask[v.id]){//此时代表车辆上一个任务计算完成，生成下一个任务，并且决策卸载
				synchronized (a1) {
					if(t.nexttime>0){
						...
						ta=vehicletaskdecision.taskdecide(ta, v);//进过决策返回结果0:自己    1：无人机    2：基站   3:通过无人机卸载到基站
						System.out.println("该任务卸载的决策为："+ta.dec.de);
						System.out.println("车辆计算能力："+v.calculation);
						System.out.println("决策完成*********************************************************");
						...
						if(ta.dec.de==1){
							...
							ta.offloadway=0;
							t.Cumulativetime=t.Cumulativetime+((ta.revolution/1024)/v.calculation)*1000;
							//System.out.println("&&&&&&&&&&&&&&&"+t.Cumulativetime);
							ta.processtime=(ta.revolution/(v.calculation*1024))*1000;
